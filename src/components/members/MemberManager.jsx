...
export default function MemberManager() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [nextMembers, nextStores] = await Promise.all([getMembers(), getStores()]);
  ...
  const save = async event => {
    ...
    try {
      ...
      setMembers(previous => editing
        ...
        : [saved, ...previous]);
      setMessage(editing ? "소속 매장이 변경되었습니다." : "구성원이 등록되었습니다. 해당 Google 계정으로 로그인하면 미션을 볼 수 있습니다.");
      resetForm();
  ...
  const toggleActive = async member => {
    ...
    try {
      ...
      setMessage(saved.is_active ? "미션 이용이 재개되었습니다." : "미션 이용이 중지되었습니다. 기존 참여 기록은 보존됩니다.");
  ...
  return <section className="member-management" aria-label="구성원 관리">
    <div className="member-heading">
      <div><h3><Users size={20} /> 구성원 관리</h3><p className="sub">Google 로그인 이메일과 소속 매장을 등록해 뷰티 미션 이용을 관리합니다.</p></div>
      <button type="button" className="soft" onClick={load} disabled={isLoading || busy}><RefreshCw size={16} /> 새로고침</button>
    </div>
    <p className="sub">한 계정에 한 매장을 지정합니다. 이용 중지는 뷰티 미션에만 적용되며 기존 참여 기록은 남습니다. 운영진은 구성원 등록 여부와 관계없이 전체 미션을 관리합니다.</p>
    ...
    <form className="card member-form" onSubmit={save}>
      ...
      <label>소속 매장<select value={storeId} onChange={event => setStoreId(event.target.value)} required disabled={busy || isLoading}>
        <option value="">매장을 선택해주세요</option>
        {stores.map(store => <option key={store.id} value={store.id}>{store.name}</option>)}
      </select></label>
      <div className="member-actions">
        <button type="submit" className="soft" disabled={busy || isLoading || !storeId || !email.trim()}>{busy ? "저장 중..." : editing ? "매장 변경 저장" : "구성원 등록"}</button>
    ...
    <div className="member-filters">
      ...
      <label>매장 필터<select value={storeFilter} onChange={event => setStoreFilter(event.target.value)}><option value="">전체 매장</option>{stores.map(store => <option key={store.id} value={store.id}>{store.name}</option>)}</select></label>
    ...
    {isLoading ? <div className="empty">구성원을 불러오는 중...</div> : filtered.length === 0 ? <div className="empty">조건에 맞는 구성원이 없습니다.</div> : <div className="member-list">
      {filtered.map(member => <article className="card member-row" key={member.id}>
        <div><strong className="member-email">{member.email}</strong><span>{stores.find(store => store.id === member.store_id)?.name || "매장 확인 필요"}</span><span className={`member-status${member.is_active ? "" : " paused"}`}>{member.is_active ? "이용 중" : "이용 중지"}</span></div>
        <div className="member-actions">
          <button type="button" className="soft" disabled={busy} onClick={() => { setEditing(member); setEmail(member.email); setStoreId(member.store_id); setMessage(""); setError(""); }}>매장 변경</button>
          <button type="button" className="soft" disabled={busy} onClick={() => toggleActive(member)}>{member.is_active ? "이용 중지" : "이용 재개"}</button>
        </div>
      </article>)}
    </div>}
  </section>;
